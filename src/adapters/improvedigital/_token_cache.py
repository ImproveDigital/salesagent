"""Bearer-token cache used by the adapter's HTTP transport.

The transport needs the same "have I got a non-expired cached token? if
not, mint a fresh one" loop as any OAuth2 client-credentials integration.
Extracted so the transport just supplies its mint function.

This is deliberately tiny -- not a full HTTP auth abstraction. The
transport keeps full control over how requests are constructed (header
shape, content-type, retry semantics); the only thing this module owns is
the cache+refresh dance over the token string.
"""
    ...
    def __init__(
        ...
        *,
        ...
        if not static_token and not mint_fn:
            raise TypeError("BearerTokenCache requires either static_token or mint_fn")

"""Unit tests for the shared adapter sync orchestration.
...
class TestSanitizeErrorMessage:
    """Admin surfaces render ``error_message`` in plain text, so adapter
    exceptions must not leak secrets or pathological traceback strings
    into admin views."""
    ...
    def test_short_clean_message_passes_through(self):
        msg = "scope: reporting scope still pending"
...
class TestSyncExecutionResultJsonPayload:
    """Canonical JSON shape shared by the per-adapter sync endpoints so
    the next adapter button doesn't reinvent another shape."""

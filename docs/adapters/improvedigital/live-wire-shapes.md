...
Validated live against the vendor's development platform (2026-08-04) via
...
- **Campaign create** (`POST /rtb/v1/classic/campaigns`) requires beyond the
  spec: `type: "Improve"`, `buying_entity_id` (the tenant's configured
  buying entity), and at least one buying entity office
  (`buying_entity_office_id`/`buying_entity_office_ids`) —
...
- **Line item create** requires `type: "Standard"`, `line_item_status:
  "Active"`, `business_unit_id` (ask the Improve Digital team for your value),
...
python scripts/ops/configure_improvedigital_tenant.py \
    ...
    --api-base-url https://api.360yield.com \
    --buying-entity-id 9001 --buying-entity-office-id 9002 \
    --business-unit-id 33 \

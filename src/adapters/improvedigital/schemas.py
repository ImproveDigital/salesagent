...
class ImproveDigitalConnectionConfig(BaseConnectionConfig):
    ...
    business_unit_id: int | None = Field(
        default=None,
        description="Business unit ID — required on Classic line items (ask the Improve Digital team for your value)",
    ...
    def _encrypt_client_secret(self, value: str | None) -> str | None:
        # Idempotent on already-encrypted input: model_dump() may run on a
        # config whose secret is still ciphertext (e.g. re-saving an unrelated
        # field) — re-encrypting would ship double-encrypted data to the
        # OAuth mint and 401.
    ...
    def _decrypt_client_secret(cls, value: str | None) -> str | None:
        # Idempotent on plaintext input (fresh from the admin form).
    ...
    def _base_url_must_be_https(cls, value: str) -> str:
        if not value.startswith("https://"):
            raise PydanticCustomError("url_scheme", "api_base_url must be an https:// URL")
        return value
    ...
    def _require_credentials(self) -> ImproveDigitalConnectionConfig:
        if not (self.client_id and self.client_secret):
            raise PydanticCustomError(
                "missing_credentials", "Improve Digital connection requires client_id + client_secret"
            )
        return self
...
class ImproveDigitalProductConfig(BaseProductConfig):
    ...
    size_ids: list[int] = Field(
        ...
        description=(
            ...
            "creative formats at buy time so the two can't diverge"
        ),
    )
    ...
    pricing_model: str | None = Field(
        default=None,
        description="Line-item pricing model (CPM confirmed; further values pending platform confirmation)",
    )
    goal: str = Field(
        default="BUDGET",
        description=(
    ...
    azerion_owned: bool | None = Field(
        default=None,
        description="Placement search default: restrict to platform-owned (azerion_owned) inventory",
    )

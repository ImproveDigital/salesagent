...
360Yield Marketplace API, targeting **Classic (direct) campaigns**:
...
class _ImproveDigitalCreativesManager:
    """Creatives-manager facade for the core approval-push path.

    ``media_buy_create`` pushes gated creatives through
    ``adapter.creatives_manager.add_creative_assets(platform_order_id, ...)``.
    The platform order ID it passes is the adapter-issued
    ``improvedigital_<campaign_id>`` buy reference, which
    :meth:`ImproveDigitalAdapter.add_creative_assets` already parses — so the
    facade is a direct delegation.
    """

    def __init__(self, adapter: ImproveDigitalAdapter) -> None:
        self._adapter = adapter

    def add_creative_assets(
        self, platform_order_id: str, assets: list[dict[str, Any]], today: datetime
    ) -> list[AssetStatus]:
        return self._adapter.add_creative_assets(str(platform_order_id), assets, today)
...
class ImproveDigitalAdapter(AdServerAdapter):
    ...
    def __init__(
        ...
        before Improve Digital issues API credentials.
        """
        ...
        # TODO: make these per-buyer — resolve from the principal's
        # improvedigital platform mapping with this config as the fallback
        # default, the same way GAM routes buyer agents to advertisers.
        ...
        if self.dry_run:
            ...
            if not (self.client_id and self.client_secret):
                raise AdCPConfigurationError(
                    "Improve Digital config requires client_id + client_secret", field="client_id"
                )
        ...
        # Core approval-push path contract: media_buy_create pushes gated
        # creatives through adapter.creatives_manager when present.
        self.creatives_manager = _ImproveDigitalCreativesManager(self)
    ...
    def _missing_booking_config(self) -> list[str]:
        ...
        if not self.buying_entity_id:
            missing.append("buying_entity_id (discover via the adapter settings' Test Connection)")
        if not self.business_unit_id:
            missing.append("business_unit_id (ask the Improve Digital team for your value)")
        return missing
    ...
    def run_inventory_sync(self) -> AdapterSyncResult:
        ...
        Called by the sync orchestration and the admin "Sync Inventory"
        button.
        """
    ...
    def create_media_buy(
        ...
        self._validate_targeting_or_raise(packages, validate_targeting, adapter_name="Improve Digital")

        buy_name = self._buy_name(request)

        if self.dry_run:
            return self._dry_run_create(request, packages, buy_name, start_time, end_time, package_pricing_info)
        ...
        try:
            ...
            # Typed errors (unresolvable geo, missing inventory selection)
            # carry their own wire semantics — clean up and let them surface.
            ...
            self._cleanup_partial_campaign(campaign_id)
            raise AdCPAdapterError(f"Improve Digital rejected the request: {exc}") from exc
    ...
    def _dry_run_create(
        ...
        """Echo the Classic campaign/line-item payloads the live path would send."""
    ...
    def _package_product(self, package: MediaPackage) -> Any | None:
        """The package's ``Product`` row, fetched once per product per
        adapter instance. Carries both ``implementation_config`` (adapter
        product config) and ``countries`` (the product page's generic
        "Target Countries" selection). Dry-run stays DB-free — returns None.
        """
        product_id = str(getattr(package, "product_id", "") or "")
        if self.dry_run or not product_id:
            return None
        ...
        if product_id not in self._product_cache:
            ...
            with get_db_session() as session:
                self._product_cache[product_id] = ProductRepository(session, self.tenant_id or "default").get_by_id(
                    product_id
                )
        return self._product_cache[product_id]

    def _product_config_from_package(self, package: MediaPackage) -> dict[str, Any]:
        """Improve Digital product config for a package.

        ``MediaPackage`` carries no implementation_config, so the config
        comes from the package's product (``Product.implementation_config``,
        under the ``improvedigital`` key or flat).
        """
        product = self._package_product(package)
        impl = (getattr(product, "implementation_config", None) or {}) if product is not None else {}
        impl = impl.get("improvedigital", impl) if isinstance(impl, dict) else {}
        return impl if isinstance(impl, dict) else {}
    ...
    def _with_product_geo_defaults(self, package: MediaPackage, product_config: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        product = self._package_product(package)
        countries = list(product.countries) if product is not None and getattr(product, "countries", None) else None
        if not countries:
    ...
    def _campaign_id_for_line_item(self, line_item_id: str) -> int | None:
        ...
        with get_db_session() as session:
            ...
            if buy is not None:
                campaign_ref = campaign_id_from_refs(
                    platform_order_ref(repo.get_packages(buy.media_buy_id)), buy.media_buy_id
                )
    ...
    def _resolve_campaign_id(self, media_buy_id: str) -> str:
        ...
        is recovered from ``package_config["platform_order_id"]``, persisted
        per package by the core layer at create. Mirror the reporting sync's
        write-side resolution so read paths hit the same cache rows.
        """
        from src.adapters.improvedigital._buy_refs import campaign_id_from_refs, platform_order_ref
        ...
        with get_db_session() as session:
            ...
            packages = repo.get_packages(media_buy_id) if buy is not None else []
        resolved = campaign_id_from_refs(platform_order_ref(packages))
        return resolved if resolved is not None else campaign_id
    ...
    def update_media_buy(
        ...
        if action not in REQUIRED_UPDATE_ACTIONS:
            self._raise_unsupported_action(action)
        ...
        if not campaign_ref.isdigit():
            raise AdCPValidationError(f"Cannot derive a Classic campaign ID from media_buy_id {media_buy_id!r}")
        campaign_id = int(campaign_ref)
        ...
        try:
            ...
            raise AdCPAdapterError(f"Improve Digital rejected the update: {exc}") from exc
    ...
    def _apply_update_action(
        ...
        if action in {"pause_media_buy", "resume_media_buy", "activate_order"}:
            ...
            if line_item_id is None:
                raise AdCPValidationError(f"Package {package_id!r} has no Improve Digital line-item mapping")
            ...
            if package_id is None or budget is None or budget <= 0:
                raise AdCPValidationError(
                    f"{action} requires a package_id and a positive value "
                    f"(got package_id={package_id!r}, value={budget!r})"
                )
            line_item_id = self._resolve_platform_line_item_id(media_buy_id, package_id)
            if line_item_id is None:
                raise AdCPValidationError(f"Package {package_id!r} has no Improve Digital line-item mapping")
    ...
    def update_media_buy_performance_index(self, media_buy_id: str, package_performance: list[Any]) -> bool:
        """Improve Digital has no native performance-index endpoint — no-op.

        Mirrors the Broadstreet precedent: log the intent (visible in
        dry-run) and report success so buyer flows that push indexes don't
        fail against platforms without the feature.
        """
        if self.dry_run:
